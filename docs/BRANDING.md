...
| the status tiers (`X-subtle`, `X-strong`, `X-fg` for danger/success/warning/info) | derived per intent hue, so an alert, badge, or toast never invents a value. `X-fg` (the text painted on `X-subtle`, and on plain surface for `.bw-field__error`/the opt-in `.bw-trend--good`/`.bw-trend--bad` sentiment modifiers (icvoss/django-brickwork#388; the `.bw-trend--up`/`--down`/`--flat` direction modifiers use `--bw-color-fg-muted` instead, since direction is not a judgement)/the account-menu's danger item) tracks `--bw-color-surface` (brickwork#289): darkening your surface darkens the status text too, so overriding surface alone does not silently drop status contrast below AA. `render_brand_css()` checks BOTH backgrounds `X-fg` is painted on (`token-manifest.json`'s `contrastPairs`) even when neither `X-fg`, `X-subtle`, nor `surface` is itself an override, and it re-resolves the check against a status base or `--bw-color-status-fg-ink` override too, so any of the three raises loudly on a definitive failure. **`X-fg` deliberately does NOT track `--bw-color-fg`**: it mixes toward a separate `--bw-color-status-fg-ink` token instead, so authoring the ordinary `fg` override above (the load-bearing seven) has no effect on status-text contrast at all. What `render_brand_css()` cannot check: a literal you author directly for `--bw-color-X-fg` or `--bw-color-X-subtle` itself, which bypasses the derivation the check verifies; that needs its own verification, the same way `fg-on-accent` does. |
