...
| Data-heavy operations | queue, audit trail, report, comparison, analysis dashboard |
...
components, 28 sections, 22 archetypes, 1 skeleton (107 items unchanged;
...
too high). Before that fix, the same 107 items were counted as 6 shells, 50
...
icvoss/django-brickwork#408 and icvoss/django-brickwork#409).
...
- But almost every ARCHETYPE (21 of 22; only `app/confirm.html` renders
  empty) needs context that is not a JSON shape at all: a
