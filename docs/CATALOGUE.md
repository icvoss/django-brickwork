...
Five kinds: the whole of D4's public grammar (Shells, Blocks, Sections,
Components) plus the one new catalogue-only noun D4 ratified (Archetype),
plus `skeleton`, added for the one file that is neither a reusable building
block nor a complete page (icvoss/django-brickwork#464). "Family" is a
separate concept, but never a catalogue NOUN in this sense: see section 3.
...
|---|---|
| Product applications | list, detail, dashboard, create/edit (form), settings, confirm, wizard, date-range-picker, console, error/maintenance, onboarding, status tracker, toast, search, activity |
| Transactional journeys | sign-in, sign-up, password reset, enrolment, checkout, review, confirmation, receipt, status tracking |
| Marketing and public web | landing, pricing, about, plan comparison, campaign, contact, conversion |
| Data-heavy operations | queue, audit trail, report, comparison, analysis dashboard, dense list, data-empty/error states |
| Documentation | documentation home, article, API reference, table of contents, navigation, search results, versioned content |
| Editorial and publishing | article, author, category, archive, series, related content, reading-progress patterns |
...
Documentation and Editorial and publishing are complete at 7 of 7; Data-heavy
operations is complete at 7 of 7; Marketing and public web is complete at 7 of 7; Transactional journeys is
complete at 7 of 7; Product applications has shipped its INTERFACE-SYSTEM
required eight (including search and activity) and sits at 15 archetypes
overall; remaining gaps live on
...
every shipped shell, component, section, archetype and skeleton: 6 shells, 83
components, 40 sections, 52 archetypes, 1 skeleton (read the shipped
`counts` object rather than this prose when the figure matters;
`examples/base.html` moved out of the archetype count into its own
`skeleton` kind rather than the item total changing, icvoss/django-brickwork#464,
correcting three releases of every published archetype figure reading one
too high). Before that fix, the equivalent 107 items were counted as 6 shells, 50
components, 28 sections, 23 archetypes (re-derived from a regenerated
manifest after the Documentation family's first two archetypes landed,
icvoss/django-brickwork#408 and icvoss/django-brickwork#409). Five further
archetypes then shipped (`app/error-maintenance.html`, `app/onboarding.html`,
`app/status-tracker.html`, `ops/dense-list.html`, `marketing/comparison.html`),
taking the archetype count from 22 to 27 and the item total from 107 to 112.
...
primitives then added `_code.html` (49, `f992b5b`, #458), a labelled,
scrollable code panel with a stated syntax-highlighting boundary (see its own
header comment): brickwork ships no tokenizer, a consumer highlights and
passes already-marked-safe markup in. `_pager.html` (50, icvoss/django-brickwork#460)
followed: a previous/next treatment for a sequential document, also a
content primitive under ADR-090's amended family rule, adding the 28th
section (`sections/content/pager.html`) alongside it. The shell count
separately moved to 6 when the docs shell landed (ADR-091, #439), which this
sentence had not previously named. The ordinals are read
...
- `name`, `kind` (`shell`/`component`/`section`/`archetype`/`skeleton`),
  `family` (the archetype's family, `null` for a cross-family shell or
  component, for every section, and for the `skeleton` kind
  (`examples/base.html`), which is a raw document skeleton tied to no
  family, not a page belonging to one);
...
- `composesItems` (on sections, archetypes and the skeleton): the inverse,
  which shells/components a given example pulls in;
- `requiresContext` (on sections, archetypes and the skeleton): whether the
  example needs render-context data to show its real content, rather than
  rendering from an empty context (see section 7).
...
- A minority need list-of-dicts or other view-supplied data a template cannot
  build inline (`features/*` list shapes, `case-list/ruled`,
  `directory/numbered`, `methods/quiet-columns`, `outcomes/quiet-columns`,
  `pricing/three-tier`,
  `pricing/comparison-table`, `stats/inline-band`, `stats/start-aligned`,
  `stats/plain-start`,
  all three `listing`
  variants, plus `portrait/*` and `bio/article-foot` image slots: matching the
  manifest's `requiresContext: true` set). THIS part could be encoded as a JSON
  shape hint (`{"features": [{"icon": "str", "heading": "str", ...}]}`), and
  would be genuinely useful.
- But almost every ARCHETYPE (26 of 28; only `app/confirm.html` and
  `app/error-maintenance.html` render empty) needs context that is not a
  JSON shape at all: a
