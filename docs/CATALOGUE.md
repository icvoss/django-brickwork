...
| Data-heavy operations | queue, audit trail |
...
every shipped shell, component, section and archetype: 5 shells, 48
components, 26 sections, 18 archetypes (97 items; re-derived from a
regenerated manifest after the scorecard/stat-comparison, queue/audit-trail
and chart-data-table work landed, 2026-08-28). The 40th component was
`_theme_switch.html` (W0.4, icvoss/django-brickwork#228), the 41st was
`_ranked_list.html` (#183), the 42nd was `_chart_card.html`, shipped after
this document's original W0.2 baseline of 39 was written, the 46th and
47th are `_scorecard.html` and `_stat_comparison.html`, and the 48th is
`_chart_data_table.html` (CHT-012).
...
- But almost every ARCHETYPE (16 of 18; only `app/confirm.html` and
...
without opening 87 files, which items are the ones that need nothing, and
