...
| `--bw-color-stage-wash` **[NEW 4.2.0]** | D | `color-mix(in oklab, var(--bw-color-accent) 8%, transparent)` | AUTHORED per BR-BW-TOK-002: `color-mix(in oklab, var(--bw-color-accent) 16%, transparent)`, doubled from light so the wash still reads against an already-dark surface. Mixed toward transparent rather than toward surface (unlike surface-marketing-tint): this is a decorative layer painted behind the copy (BR-BW-MKT-007), not a background fill, so alpha alone carries the effect |
| `--bw-color-stage-grid` **[NEW 4.2.0]** | D | `color-mix(in oklab, var(--bw-color-border) 60%, transparent)` | AUTHORED per BR-BW-TOK-002: `color-mix(in oklab, var(--bw-color-border) 70%, transparent)`, a stronger line so the grid stays visible on graphite |
...
`.bw-trend--good`/`.bw-trend--bad` (and their retained aliases), and
`.bw-account-menu__item--danger` actually use) by
...
| `--bw-component-docs-nav-inline-size` **[NEW, #671]** | `16rem` | Docs shell rail column width at the lg breakpoint (`.bw-docs-layout` `grid-template-columns`). Override when document titles need a wider rail; do not rewrite the grid |
