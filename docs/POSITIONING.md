...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 218 documents across two fixture sets: 152 hand-maintained pages (76 fixtures x light and dark) plus 66 catalogue-document pages (33 catalogue documents, being 32 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Fact | Value | Note |
|---|---|---|
| Components | 67 | 50 core, 17 marketing. **Gated** against `catalogue-manifest.json` |
...
| Archetypes | 32 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
| Template tag registrations | 30 total | 19 `inclusion_tag`, 10 `simple_tag`, 1 `filter`. Write "19 component tags" or state the 30 total; never a bare "19 template tags". **Gated** by importing the templatetags libraries and counting `register.tags`/`register.filters` |
...
| Examples | 61 | 32 archetype pages, 29 sections. **Gated** against `catalogue-manifest.json` |
