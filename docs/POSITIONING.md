...
Marketing and public web 7/7 required archetypes, with Product applications
the deepest family (13 archetypes), Data-heavy operations at 6, and
Transactional journeys at 3. Remaining INTERFACE-SYSTEM gaps concentrate in
transactional journeys (plus specialist viz fitting the data contract). Cite
section 5 gated counts when quoting inventory.
...
**The composition evidence.** 74 of the 75 examples are pure composition: zero
...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 242 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 88 catalogue-document pages (44 catalogue documents, being 43 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Archetypes | 43 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
...
| Examples | 75 | 43 archetype pages, 32 sections. **Gated** against `catalogue-manifest.json` |
