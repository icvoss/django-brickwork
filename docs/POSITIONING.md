...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 240 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 86 catalogue-document pages (43 catalogue documents, being 42 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Archetypes | 42 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
...
| Examples | 74 | 42 archetype pages, 32 sections. **Gated** against `catalogue-manifest.json` |
