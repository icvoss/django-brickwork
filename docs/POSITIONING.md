...
required archetypes, with Product applications the deepest family (15
archetypes), Data-heavy operations at 7/7, Marketing and public web at 7/7, and
Transactional journeys at 7/7. Remaining INTERFACE-SYSTEM gaps concentrate in
...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 260 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 106 catalogue-document pages (53 catalogue documents, being 52 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
`--bw-*` tokens exist (dated at 3.12.0), 313 overridable. 10 are load-bearing,
...
| Fact | Value | Note |
|---|---|---|
| Components | 84 | 58 core, 26 marketing. **Gated** against `catalogue-manifest.json` |
| Shells | 6 | base, app, auth, centred, docs, marketing. **Gated** against `catalogue-manifest.json` |
| Sections | 44 | **Gated** against `catalogue-manifest.json` |
...
| Tokens | 358 unique `--bw-*` | 313 overridable; 10 load-bearing, 8 unconditional. **Overridable count gated** against `token-manifest.json`; the 358 total (all custom properties in compiled `tokens.css`) is **dated** at 3.12.0 |
...
| Examples | 96 | 52 archetype pages, 44 sections. **Gated** against `catalogue-manifest.json` |
...
| A11y gate | 274 axe-scanned documents | 168 hand-maintained (84 fixtures x light and dark) plus 106 catalogue-document (53 catalogue documents x light and dark), blocking CI. Catalogue-document fixture count **gated** against `catalogue-manifest.json`'s archetype plus skeleton counts; the 168 hand-maintained fixtures are **gated** against the real `a11y/generate_fixtures.py` run's own written output by `tests/test_a11y_fixture_coverage.py` (icvoss/django-brickwork#226), so this figure can never drift from what the axe gate actually loads. Hand-maintained fixture COVERAGE (does every shell/component/section have at least one fixture) is separately **gated** by the same file against `catalogue-manifest.json`. The 53 to 55 fixture-file step (icvoss/django-brickwork#272, plus icvoss/django-brickwork#275's `bw_ranked_list` fixture landing independently on `main`) added `theme-switch-compact-<theme>.html`, the layout="compact" no-JS floor, and the ranked-list fixture: the pre-existing no-JS coverage only ever rendered layout="inline". The 55 to 56 step (icvoss/django-brickwork#185) added `data-table-empty-cta-<theme>.html`, the `_data_table.html` empty-state action CTA. The 56 to 57 step (the chart card work) added `chart-card-<theme>.html`, covering the real `{% bw_chart_mount %}` tag's accessible-name pairing plus the card's loading, error and empty states. The 57 to 58 step (the sparkline work) added `sparkline-<theme>.html`, covering both tones, the highlight marker and the no-JS floor. The 58 to 59 step (the trend indicator work, VIZ-017) added `trend-indicator-<theme>.html`, covering the up/down/flat states of the standalone `_trend_indicator.html` partial extracted from `_stat.html`'s own trend block. The 60 to 61 step (the scorecard/stat-comparison work, VIZ-011/012/019/020) added `scorecard-<theme>.html`, covering the shared dashboard grid's span= modifiers arranging real `_stat.html` cards, plus `_stat_comparison.html`'s sm/md/lg sizes each paired with a different trend direction. The 61 to 62 step (ADR-091, icvoss/django-brickwork#439) added `docs-<theme>.html`, the new docs shell's own fixture: a populated two-column [article | rail] page with real long-form `.bw-prose` content and a real `{% bw_nav %}` rail, the shell that had no fixture at all until this step. The 62 to 63 step (icvoss/django-brickwork#448 item 1) added `docs-with-site-chrome-<theme>.html`, exercising `docs_site_header_region`/`docs_site_footer_region`, the site-wide chrome seam outside `<main>` no other docs fixture had rendered. The 63 to 65 step (the code-display work, icvoss/django-brickwork#259) added a `code-display-<theme>.html`/`code-display-js-<theme>.html` pair: a plain panel, a filename+language panel and a copyable panel with real multi-line source, plus the JS leg proving bwCodeCopy's own init() reveals the copy control the no-JS floor ships hidden. The 65 to 66 step (the mobile-nav toggle work, icvoss/django-brickwork#263) added `mobile-nav-toggle-<theme>.html`, covering the marketing_nav_region include pattern. The 66 to 67 step (appearance suite card pilot, icvoss/django-brickwork#535) added `card-<theme>.html`, covering surface/elevation and header_recipe/footer_recipe chrome on `_card.html`. The 68 to 73 step (marketing header overlay, icvoss/django-brickwork#565) added five `marketing-overlay-*` fixtures per theme (no-JS opaque floor, stamped dark/light-scrolled/dark-scrolled contexts, plus the JS PE boot leg). The 73 to 74 step (token specimen, icvoss/django-brickwork#268) added `token-specimen-<theme>.html`, the dual-pane load-bearing brand token grid with contrast-pair chips. The 74 to 75 step (preview frame, icvoss/django-brickwork#269) added `preview-frame-<theme>.html`, the surface-guarded live-render container. The 75 to 76 step (proof collage, icvoss/django-brickwork#572) added `proof-collage-<theme>.html`, the marketing kit collage / page-as-proof section. The 76 to 77 step (version switcher, icvoss/django-brickwork#414) added `version-switch-<theme>.html`, covering `{% bw_version_switch %}` current-version disclosure, aria-current marking and latest/deprecated status text. The 77 to 78 step (docs toc, icvoss/django-brickwork#627) added `toc-<theme>.html`, covering `{% bw_toc %}` on-this-page landmark, nested `__sub` entries and aria-current="location". The 78 to 79 step (three-region app shell, icvoss/django-brickwork#700 to #703) added `app-regions-<theme>.html`, covering `layout="regions"` with an icon-density rail (link plus menu trigger), contextual sidebar and empty-track reclaim. The 79 to 83 step (marketing sustainment, BR-BW-MKT-007..009, icvoss/django-brickwork#570/#571) added four fixture pairs: `soft-stage-overlay-<theme>.html`, composing the soft-stage atmosphere with the overlay header on the same document (BR-BW-MKT-007 rule 3); `soft-stage-media-behind-<theme>.html`, composing soft-stage with media_placement="behind" (BR-BW-MKT-007 rule 3, ADR-057 section 1a; the stacking-order proof is a11y/marketing_stacking.spec.mjs); `product-shot-placement-<theme>.html`, the product-shot include inside a hero media slot at both "beside" and the "below" default (BR-BW-MKT-008); and `marketing-reveal-<theme>.html`, a section with reveal="enter" wrapping a feature grid, the resting (no-JS) export (BR-BW-MKT-009; the reduced-motion computed-style proof is a11y/marketing_reveal.spec.mjs). The 83 to 84 step (hero width="bleed", icvoss/django-brickwork#710) added `hero-bleed-soft-stage-<theme>.html`, composing width="bleed" with atmosphere="soft-stage" under the overlay header (the geometry proof is a11y/marketing_bleed.spec.mjs). The archetype half then moved from 32 to 36 (16 to 18 catalogue archetypes) when the Data-heavy operations family shipped `queue.html` and `audit-trail.html`, then from 36 to 40 (18 to 20 catalogue archetypes) when the same family shipped `report.html` and `comparison.html`, then from 40 to 42 (20 to 21 catalogue archetypes) when it shipped `analysis-dashboard.html`, then from 42 to 46 (21 to 23 catalogue archetypes) when the Documentation family opened with `docs/home.html` and `docs/article.html` (icvoss/django-brickwork#408, icvoss/django-brickwork#409). The count then held at 46 when `examples/base.html` was reclassified from `archetype` to its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, so the published archetype figure drops from 23 to 22, but the document is still scanned. The archetype half then moved from 46 to 56 catalogue-document pages (23 to 28 catalogue documents) when five further archetypes shipped: `app/error-maintenance.html`, `app/onboarding.html`, `app/status-tracker.html`, `ops/dense-list.html` and `marketing/comparison.html`. Beat Phase D then moved the catalogue-document half from 56 to 58 pages (28 to 29 catalogue documents) by adding the toast journey archetype `app/toast.html` (the modal journey ships as section `sections/modal/confirm.html`, so it does not add a catalogue-document page). The Documentation family's API reference, table of contents, navigation and search-results archetypes (icvoss/django-brickwork#410, #413, #412, #411) then moved the catalogue-document half from 58 to 66 pages (29 to 33 catalogue documents). Versioned content (icvoss/django-brickwork#414) then moved it from 66 to 68 pages (33 to 34 catalogue documents), completing Documentation at 7 of 7. The Editorial family's article archetype (icvoss/django-brickwork#415) then moved the catalogue-document half from 68 to 70 pages (34 to 35 catalogue documents), opening Editorial at 1 of 7. The Editorial author archetype (icvoss/django-brickwork#416) then moved it from 70 to 72 pages (35 to 36 catalogue documents), Editorial at 2 of 7. The Editorial category archetype (icvoss/django-brickwork#417) then moved it from 72 to 74 pages (36 to 37 catalogue documents), Editorial at 3 of 7. The Editorial archive archetype (icvoss/django-brickwork#418) then moved it from 74 to 76 pages (37 to 38 catalogue documents), Editorial at 4 of 7. The Editorial series archetype (icvoss/django-brickwork#419) then moved it from 76 to 78 pages (38 to 39 catalogue documents), Editorial at 5 of 7. The Editorial related-content archetype (icvoss/django-brickwork#420) then moved it from 78 to 80 pages (39 to 40 catalogue documents), Editorial at 6 of 7. The Editorial reading-progress archetype (icvoss/django-brickwork#421) then moved it from 80 to 82 pages (40 to 41 catalogue documents), completing Editorial at 7 of 7. The Data-heavy operations data-empty/error archetype (icvoss/django-brickwork#407) then moved it from 82 to 84 pages (41 to 42 catalogue documents), completing Data-heavy operations at 7 of 7. The Product applications search and activity archetypes (icvoss/django-brickwork#404, #405) then moved it from 84 to 88 pages (42 to 44 catalogue documents), Product applications at 15 archetypes. The Marketing campaign, contact and conversion archetypes (icvoss/django-brickwork#400, #402, #403) then moved it from 88 to 94 pages (44 to 47 catalogue documents), completing Marketing and public web at 7 of 7. The Transactional enrolment, checkout, review, confirmation, receipt and status-tracking archetypes (icvoss/django-brickwork#422 to #427) then moved it from 94 to 106 pages (47 to 53 catalogue documents), completing Transactional journeys at 7 of 7. `a11y/generate_archetype_fixtures.py` discovers over every whole-document kind in its own `_SCANNED_KINDS` rather than over `archetype` alone, precisely so correcting a count cannot silently withdraw a surface from the axe, no-JS and keyboard gates |
...
| Version | 4.2.0 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
