...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 228 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 74 catalogue-document pages (37 catalogue documents, being 36 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Archetypes | 36 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
...
| Examples | 65 | 36 archetype pages, 29 sections. **Gated** against `catalogue-manifest.json` |
