...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 212 documents across two fixture sets: 152 hand-maintained pages (76 fixtures x light and dark) plus 60 catalogue-document pages (30 catalogue documents, being 29 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Archetypes | 29 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
...
| Examples | 58 | 29 archetype pages, 29 sections. **Gated** against `catalogue-manifest.json` |
