...
by touching component classes. 352 unique `--bw-*` tokens exist (dated at
...
| Tokens | 352 unique `--bw-*` | 283 overridable; 10 load-bearing, 8 unconditional. **Overridable count gated** against `token-manifest.json`; the 352 total (all custom properties in compiled `tokens.css`) is **dated** at 3.12.0 |
