...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 190 documents across two fixture sets: 134 hand-maintained pages (67 fixtures x light and dark) plus 56 catalogue-document pages (28 catalogue documents, being 27 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Fact | Value | Note |
|---|---|---|
| Components | 51 | 41 core, 10 marketing. **Gated** against `catalogue-manifest.json` |
...
| Template tag registrations | 27 total | 17 `inclusion_tag`, 9 `simple_tag`, 1 `filter`. Write "17 component tags" or state the 27 total; never a bare "17 template tags". **Gated** by importing the templatetags libraries and counting `register.tags`/`register.filters` |
...
| Examples | 55 | 27 archetype pages, 28 sections. **Gated** against `catalogue-manifest.json` |
...
| Version | 3.22.0 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
