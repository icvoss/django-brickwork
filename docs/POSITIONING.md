...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 186 documents across two fixture sets: 130 hand-maintained pages (65 fixtures x light and dark) plus 56 catalogue-document pages (28 catalogue documents, being 27 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Fact | Value | Note |
|---|---|---|
| Components | 50 | 41 core, 9 marketing. **Gated** against `catalogue-manifest.json` |
...
| Template tag registrations | 25 total | 17 `inclusion_tag`, 7 `simple_tag`, 1 `filter`. Write "17 component tags" or state the 25 total; never a bare "17 template tags". **Gated** by importing the templatetags libraries and counting `register.tags`/`register.filters` |
