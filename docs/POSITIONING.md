...
because the package ships 51 examples (23 pages, 28 sections) built from
...
**The composition proof.** 50 of the 51 examples are pure composition: zero
...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 176 documents across two fixture sets: 130 hand-maintained pages (65 fixtures x light and dark) plus 46 archetype pages (23 catalogue archetypes x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Archetypes | 23 | **Gated** against `catalogue-manifest.json` |
...
| Examples | 51 | 23 archetype pages, 28 sections. **Gated** against `catalogue-manifest.json` |
...
| Version | 3.15.0 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
