...
`--bw-*` tokens exist (dated at 3.12.0), 311 overridable. 10 are load-bearing,
...
| Fact | Value | Note |
|---|---|---|
| Components | 83 | 58 core, 25 marketing. **Gated** against `catalogue-manifest.json` |
| Shells | 6 | base, app, auth, centred, docs, marketing. **Gated** against `catalogue-manifest.json` |
| Sections | 40 | **Gated** against `catalogue-manifest.json` |
...
| Tokens | 358 unique `--bw-*` | 311 overridable; 10 load-bearing, 8 unconditional. **Overridable count gated** against `token-manifest.json`; the 358 total (all custom properties in compiled `tokens.css`) is **dated** at 3.12.0 |
...
| Examples | 92 | 52 archetype pages, 40 sections. **Gated** against `catalogue-manifest.json` |
