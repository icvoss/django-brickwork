...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 236 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 82 catalogue-document pages (41 catalogue documents, being 40 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Fact | Value | Note |
|---|---|---|
| Components | 68 | 51 core, 17 marketing. **Gated** against `catalogue-manifest.json` |
...
| Archetypes | 40 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
...
| Examples | 69 | 40 archetype pages, 29 sections. **Gated** against `catalogue-manifest.json` |
...
| Version | 3.31.0 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
