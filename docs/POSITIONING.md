...
line. At 3.33.0 the catalogue ships Documentation 7/7, Editorial 7/7, and
Transactional journeys 7/7 required archetypes (9 pages in `examples/auth/`,
including sign-up and password reset as extras beyond the required seven),
with Product applications the deepest family (13 archetypes), Data-heavy
operations at 6, and Marketing and public web at 4. Remaining INTERFACE-SYSTEM
gaps concentrate in marketing/public-web breadth (plus specialist viz fitting
the data contract). Cite section 5 gated counts when quoting inventory.
...
certified consumer outcome. "Anything" overclaims partial marketing coverage
and must not return as an H1 hedge.
...
| What runs | Scope |
|---|---|
| axe-core WCAG 2.2 AA scan | 248 documents across two fixture sets: 154 hand-maintained pages (77 fixtures x light and dark) plus 94 catalogue-document pages (47 catalogue documents, being 46 archetypes plus 1 skeleton, x light and dark), blocking every push (`a11y-gate` CI job) |
...
| Fact | Value | Note |
|---|---|---|
| Components | 70 | 51 core, 19 marketing. **Gated** against `catalogue-manifest.json` |
...
| Archetypes | 46 | **Gated** against `catalogue-manifest.json`. `examples/base.html` moved out of this count into its own `skeleton` kind (icvoss/django-brickwork#464): it is a raw document skeleton a consumer copies, not a complete page, and was the only archetype ever carrying `family: null` |
| Template tag registrations | 31 total | 20 `inclusion_tag`, 10 `simple_tag`, 1 `filter`. Write "20 component tags" or state the 31 total; never a bare "20 template tags". **Gated** by importing the templatetags libraries and counting `register.tags`/`register.filters` |

| Tokens | 358 unique `--bw-*` | 289 overridable; 10 load-bearing, 8 unconditional. **Overridable count gated** against `token-manifest.json`; the 358 total (all custom properties in compiled `tokens.css`) is **dated** at 3.12.0 |
...
| Examples | 78 | 46 archetype pages, 32 sections. **Gated** against `catalogue-manifest.json` |
...
| Version | 3.33.0 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
