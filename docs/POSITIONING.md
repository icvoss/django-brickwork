...
| Template tag registrations | 26 total | 17 `inclusion_tag`, 8 `simple_tag`, 1 `filter`. Write "17 component tags" or state the 26 total; never a bare "17 template tags". **Gated** by importing the templatetags libraries and counting `register.tags`/`register.filters` |
...
| Version | 3.19.1 | consistent in `pyproject.toml` and `src/brickwork/__init__.py`. **Gated** |
