...
- **Stays consumer-owned:** product-specific semantics, business rules,
  ...
  consumer owns, so it is copied in and edited, never extended.
  ...
  of components and shells, not a CMS. The marketing kit is the first slice of a
  wider templates-catalogue trajectory (layout variants, then themed starter
  kits, each demand-gated); see
  `oss/docs/plans/brickwork-templates-catalogue-direction.md` in the umbrella.
