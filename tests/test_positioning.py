...
def test_a11y_gate_archetype_fixture_count_matches_the_shipped_manifest() -> None:
    ...
    The mobile-nav toggle work (icvoss/django-brickwork#263) then added a
    mobile-nav-toggle fixture pair covering the marketing_nav_region include
    ...
    list_item, moving the real count to 68/136.) The marketing header overlay
    work (icvoss/django-brickwork#565) then added five marketing-overlay
    fixture pairs, moving the real count to 73/146.) The token specimen work
    (icvoss/django-brickwork#268) then added a token-specimen fixture pair,
    moving the real count to 74/148.) The preview frame work
    (icvoss/django-brickwork#269) then added a preview-frame fixture pair,
    moving the real count to 75/150.) The proof collage work
    (icvoss/django-brickwork#572) then added a proof-collage fixture pair,
    moving the real count to 76/152.) The version-switch work
    (icvoss/django-brickwork#414) then added a version-switch fixture pair,
    moving the real count to 77/154.) The docs toc work
    ...
    (BR-BW-MKT-007..009, icvoss/django-brickwork#570/#571) then added three
    fixture pairs (soft-stage-overlay, product-shot-placement,
    marketing-reveal), moving the real count to 82/164.)
    ...
    assert _leading_int(value) == 164 + (document_count * 2)
