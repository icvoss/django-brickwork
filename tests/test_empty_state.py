...
Covers the context-variable path, the concise blocks (icon/heading/body/action),
and the 4.0.0 removal of deprecated title/description/empty_state_action fills.
"""
...
def test_removed_title_block_no_longer_replaces_the_heading() -> None:
    # On empty_state, title was the deprecated predecessor of heading; removed
    # in 4.0.0. Filling it is discarded (Django silent discard), so the
    # heading context remains.
    out = _extend("{% block title %}TITLE-SENTINEL{% endblock %}")
    assert "TITLE-SENTINEL" not in out
    assert "No invoices yet" in out


def test_removed_description_block_no_longer_replaces_the_body() -> None:
    out = _extend("{% block description %}DESC-SENTINEL{% endblock %}")
    assert "DESC-SENTINEL" not in out
    assert "Create your first invoice to get started." in out


def test_heading_wins_and_legacy_title_fill_is_discarded() -> None:
    out = _extend("{% block heading %}HEADING-SENTINEL{% endblock %}{% block title %}TITLE-SENTINEL{% endblock %}")
    assert "HEADING-SENTINEL" in out
    assert "TITLE-SENTINEL" not in out


def test_body_wins_and_legacy_description_fill_is_discarded() -> None:
    out = _extend("{% block body %}BODY-SENTINEL{% endblock %}{% block description %}DESC-SENTINEL{% endblock %}")
    assert "BODY-SENTINEL" in out
    assert "DESC-SENTINEL" not in out


# --- action block (empty_state_action removed in 4.0.0) ----------------------
...
def test_legacy_empty_state_action_fill_is_discarded_when_action_is_filled() -> None:
    out = _extend(
...
def test_empty_state_action_alone_no_longer_renders() -> None:
