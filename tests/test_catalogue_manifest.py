...
1. **Manifest shape**: the typed reader's accessors match the raw JSON, and
   the documented counts hold (6 shells, 64 components, 28 sections, 22
...
def test_items_by_kind_filters_correctly() -> None:
    ...
    assert len(items_by_kind("component")) == 83
    assert len(items_by_kind("section")) == 40
...
def test_sections_requiring_context_match_the_documented_shape() -> None:
    ...
    assert section_names == {
        "sections/bio/article-foot.html",
        "sections/case-list/ruled.html",
        "sections/directory/numbered.html",
        ...
        "sections/outcomes/quiet-columns.html",
        ...
        "sections/stats/start-aligned.html",
        "sections/stats/plain-start.html",
...
def test_most_sections_render_from_empty_context() -> None:
    ...
    assert len(empty_context) == 22  # 40 sections total, 18 need context
