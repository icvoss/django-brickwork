...
def test_items_by_kind_filters_correctly() -> None:
    ...
    assert len(items_by_kind("component")) == 70
    assert len(items_by_kind("section")) == 32
    assert len(items_by_kind("archetype")) == 41
...
def test_most_sections_render_from_empty_context() -> None:
    ...
    assert len(empty_context) == 20  # 32 sections total, 12 need context
