...
- **Beat F.4 claim cite.** POSITIONING and README may now state prefer-where-
  evidenced beat language with a cite to
  `docs/audits/2026-09-14-beat-win-scorecard-s5s7.md` (6/8 lead or tie). Meet-
  bar PASS cite is retained. Honesty held: S5 remains Lose; S3 remains N/S;
  no soft-pass.
...
theme switch that affects any consumer placing it in a visible header.

**What changes for you on upgrade.** If you place `{% bw_theme_switch %}` in a
header, the control now reserves its box before JavaScript runs instead of
appearing after first paint, so the page no longer jumps. Two consequences: a
visitor without JavaScript now sees an invisible reserved space where the
control would be rather than nothing at all, and the `hidden` attribute is gone
from the control's pre-init state, so a CSS selector or test keyed to that
attribute stops matching. The migration is one line and is described in the
Fixed entry below. Nothing else in the public contract moves: option names,
...
catalogue machine-readable and the width story single-sourced. Breakpoints
become real tokens with the 768px boundary defect fixed, every shipped item
gains a catalogue-manifest entry a consumer can read off the installed
package, the four theming axes get a supported live on-page control, and
every full-page example is now swept by an auto-discovering harness across
all supported widths and both themes in CI. Upgrade notes for existing
consumers: two exact viewport widths change behaviour (768px and 640px, see
Changed and Fixed), the shipped `brickwork.css` is reshaped throughout by
the full Tailwind compile pass (re-baseline any byte-wise diffing; rendering
changes only in browsers without `color-mix()` support), and two
accessibility defects in the shipped examples are fixed at source.
...
- **`--bw-breakpoint-sm/md/lg/xl` ship as real tokens, replacing 15 hardcoded
  ...
  width another component's desktop layout was already active. At exactly
  768px, exactly one side of the corrected boundary now applies, verified
  by browser measurement. The honest limit:
