...
| `semantic.light.tokens.json` | Semantic, light | Role-named `--bw-color-*` (surface, fg, border, accent, status). Names ARE the public contract; values are brand-overridable. |
| `semantic.dark.tokens.json` | Semantic, dark | The **authored** dark values (BR-BW-TOK-002): same names, deliberately chosen dark oklch values, never derived from light. |
| `component.tokens.json` | Component | The few per-component tokens (icon size/stroke, button radius, disabled opacity) distinct from their semantic default. |
...
- **Theme**: `:root` / `[data-theme="light"]` carry the light colours;
  `[data-theme="dark"]` overrides only the `--bw-color-*` tokens (dark authored,
  BR-BW-TOK-002).
