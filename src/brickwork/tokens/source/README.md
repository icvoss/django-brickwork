...
| `semantic.dark.tokens.json` | Semantic, dark | The **authored** dark values (BR-BW-TOK-002): same names, deliberately chosen dark oklch values, never derived from light. Authored applies to the load-bearing set; within-theme fine tokens derive from it with dark-tuned constants, and a brand may override any of them. |
...
- **Brand**: a consumer overrides the `--bw-*` names in its own stylesheet,
  loaded after `tokens.css` (the shipped mechanism; docs/BRANDING.md is the
  guide). Because derived tokens are live color-mix expressions, overriding a
  load-bearing token recolours its derived family with no rebuild.
...
- **BR-BW-TOK-002**: load-bearing dark values are authored in
  `semantic.dark.tokens.json`, never computed from light; within-theme fine
  tokens derive with dark-tuned constants (docs/DESIGN.md section 3).
...
artefacts; `tests/test_token_derivations.py` recomputes every derived
color-mix expression and asserts it stays within tolerance of its `$value`
baseline.
