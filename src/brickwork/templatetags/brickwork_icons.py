...
# size arg -> the --bw-icon-size-* token the SVG's width/height resolves to
# (ICO-004: sm 16 / md 20 / lg 24 / xl 32, aligned to context). The value is a
# CSS var reference, so the actual px is token-driven and density/brand aware.
...
def bw_icon(
    ...
    *,
    ...
    svg = (
        f'<svg class="{classes}" '
        f'width="{_SIZE_TOKENS[size]}" height="{_SIZE_TOKENS[size]}" '
