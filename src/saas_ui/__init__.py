"""saas_ui: a brand-agnostic, app-facing professional UI substrate for Django.
...
direction). See docs/specs/saas-ui/ in the icvoss/oss umbrella for the five
versioned public-API contracts (token, template, navigation, interaction,
JavaScript).

The design-token layer lives in the framework-neutral ``saas_ui.tokens``
sub-module; a separate ``saas_ui_tokens`` package is deferred until a
non-Django consumer earns the split.
"""

__version__ = "0.0.0"

default_app_config = "saas_ui.apps.SaasUiConfig"
